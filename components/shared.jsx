    ...
    <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${scrolled ? 'bg-paper/90 backdrop-blur-md border-b border-line shadow-[0_4px_20px_-12px_rgba(0,73,83,0.12)]' : 'bg-transparent'}`}>
      ...
      <div className={`md:hidden overflow-hidden transition-[max-height] duration-300 ${mobileOpen ? 'max-h-[600px]' : 'max-h-0'} bg-paper border-b border-line`}>
        <div className="px-6 py-4 flex flex-col gap-1">
          {links.map((l) => (
            <Link key={l.href} href={l.href}
              className={`py-3 px-3 rounded-lg font-sans text-[15px] font-medium ${isActive(l.href) ? 'text-paper bg-midnight' : 'text-ink'}`}>
              {l.label}
            </Link>
          ))}
          <Link href="/contact" className="btn-primary mt-2 text-center px-5 py-3 rounded-full font-sans text-[14px] font-semibold bg-midnight text-paper block">Let's Talk</Link>
        </div>
      </div>
    </header>
  );
      ...
      <div className="max-w-[1280px] mx-auto px-6 md:px-10 pt-20 pb-10 relative">
        <div className="grid md:grid-cols-12 gap-10 md:gap-8">
          ...
          <div className="md:col-span-5">
            ...
            <ul className="space-y-3 text-[14px]">
              {[['About Us', '/about'], ['Blog', '/blog'], ['Contact', '/contact'], ['Privacy Policy', '/privacy']].map(([label, href]) => (
                <li key={href}>
                  <Link href={href} className="text-paper/80 hover:text-cyan transition-colors">{label}</Link>
                </li>
              ))}
            </ul>
          </div>

...
import { useRouter, useReveal, Section, Eyebrow, Banner } from './shared.jsx';
...
function BlogPage() {
  ...
  const slugMatch = path.startsWith('/blog/') ? path.slice('/blog/'.length) : null;
  const post = slugMatch ? BLOG_POSTS.find((p) => p.slug === slugMatch) : null;
    ...
    <div>
      ...
      {featured && (
        <section className="pb-16">
          <Section>
            <button onClick={() => go(`/blog/${featured.slug}`)} className="reveal block w-full text-left card-hover rounded-3xl border border-line bg-paper overflow-hidden grid md:grid-cols-12">
      ...
      <section className="pb-32">
        <Section>
          <div className="grid md:grid-cols-3 gap-6">
            {filtered.map((p, i) => (
              <button key={p.slug} onClick={() => go(`/blog/${p.slug}`)}
...
function BlogDetail({ post }) {
  const { go } = useRouter();
  useReveal();
  useEffect(() => { scrollTo(0, 0); }, [post.slug]);
  ...
  return (
    <article>
      <section className="pt-[130px] pb-10 md:pt-[160px] md:pb-14">
        <Section>
          <button onClick={() => go('/blog')} className="font-sans text-[13px] font-medium text-midnight inline-flex items-center gap-1.5 hover:gap-2 transition-all reveal">
      ...
      <section className="pb-32 bg-cyan-soft/40 py-20">
        <Section>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {related.map((p) => (
              <button key={p.slug} onClick={() => go(`/blog/${p.slug}`)} className="reveal text-left card-hover rounded-2xl border border-line bg-paper overflow-hidden">
                <Banner src={p.img} alt={p.title} ratio="16/10" />
                <div className="p-6">

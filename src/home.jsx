...
function ServiceFlipCard({ s, onClick }) {
  return (
    <div className="flip-card h-[360px] cursor-pointer" onClick={onClick}>
          ...
          {steps.map((step, i) => (
            ...
            >
              <div className="relative aspect-[16/10] overflow-hidden bg-midnight/10">
                ...
                <span className="absolute top-4 left-4 font-mono text-[11px] tracking-[0.2em] text-paper/90 bg-midnight/50 backdrop-blur-sm px-3 py-1 rounded-full">
                  {step.n}
                </span>
              </div>
    ...
    <Section className="mt-2 md:mt-6 pb-16 md:pb-20">
      ...
      >
        <div className="absolute inset-0 pointer-events-none" aria-hidden>
          ...
          <svg className="absolute inset-0 h-full w-full opacity-[0.22]" viewBox="0 0 1200 360" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function HomePage() {
  ...
  return (
    <div className="relative isolate">
      {/* Vercel-style ambient: full scroll height behind all home sections */}
      ...
      <section className="pt-[120px] pb-0 md:pt-[150px] md:pb-0 relative">
        <Section>
          <motion.div
            className="max-w-3xl pb-12 md:pb-14"
            initial="hidden"
            animate="visible"
            variants={stagger}
          >
            ...
            {[
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6 md:p-7">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-cyan-soft text-midnight mb-4">
                      <Icon />
                    </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {SERVICES.map((s) => (
              <div key={s.id} className="reveal flip-card-shell h-[360px]">
                <ServiceFlipCard s={s} onClick={() => go(`/services#${s.id}`)} />
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section>
          <div className="grid md:grid-cols-12 gap-10 lg:gap-16 items-start relative">
            <div className="md:col-span-5 reveal">
              ...
              <h2 className="mt-5 font-display font-extrabold text-[32px] md:text-[48px] leading-[1.05]">
                Tell us what you are building.
              </h2>
              <p className="mt-5 text-[15px] leading-relaxed text-paper/75 max-w-md">
      ...
      <Section>
        <div className="reveal mb-10 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button type="button" onClick={() => go('/blog')} className="ml-1 btn-ghost inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-sans text-[13px] font-semibold">
              View all <span>→</span>
            </button>
          </div>
        </div>
        ...
        <div ref={scrollerRef} className="reveal flex gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory pb-4 -mx-6 px-6">
          {latest.map((p) => (
            <button key={p.slug} type="button" data-card onClick={() => go(`/blog/${p.slug}`)}

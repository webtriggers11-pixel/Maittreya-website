import { useReveal, Section, Eyebrow, LeadForm } from './shared.jsx';
        ...
        <Section>
          <div className="max-w-3xl reveal">
            ...
            <p className="mt-7 text-[17px] leading-relaxed text-ink-soft max-w-xl">
              We reply within a working day — often with sharper questions before booking a call. The more context you share, the better.
            </p>
          </div>
        </Section>
      </section>
            ...
            <aside className="md:col-span-5 space-y-6">
              ...
              <div className="reveal rounded-2xl bg-midnight text-paper p-7" style={{ transitionDelay: '80ms' }}>
                ...
                <div className="mt-6 grid grid-cols-3 gap-3 text-[12px]">
                  {[['Mumbai','IST']].map(([c,z]) => (
                    <div key={c} className="rounded-lg bg-midnight-700 p-3">
                      <div className="font-display font-semibold text-paper">{c}</div>
                      <div className="font-mono text-paper/60 mt-0.5">{z}</div>
                    </div>
                  ))}
                </div>
              </div>
